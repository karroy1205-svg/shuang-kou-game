...
let tConfig = null; // 本地追踪进贡状态

const dom = {
    lobby: document.getElementById('lobby-screen'), settlement: document.getElementById('settlement-screen'),
    ...
    btns: { draw: document.getElementById('draw-btn'), call: document.getElementById('call-btn'), over: document.getElementById('override-btn'), want: document.getElementById('want-btn'), take: document.getElementById('take-bottom-btn'), bury: document.getElementById('bury-btn'), payT: document.getElementById('pay-tribute-btn'), retT: document.getElementById('return-tribute-btn'), play: document.getElementById('play-btn') }
};
...
function addLog(msg) { let li = document.createElement('li'); li.innerText = msg; dom.chatList.appendChild(li); dom.chatList.scrollTop = dom.chatList.scrollHeight; }
...
function updateUI() {
    ...
    Object.values(dom.btns).forEach(btn => btn.style.display = 'none'); // 强制净化，先全隐藏
    ...
    // 进贡按钮显隐控制 (安全解耦，防闪烁)
    if (gState === 'TRIBUTE_PAY' && !amISpectator && tConfig && tConfig.payers.includes(myIdx)) dom.btns.payT.style.display = 'inline-block';
    if (gState === 'TRIBUTE_RETURN' && !amISpectator && tConfig && tConfig.receivers.includes(myIdx)) dom.btns.retT.style.display = 'inline-block';
...
document.getElementById('confirm-settlement-btn').onclick = () => { socket.emit('ackSettlement'); document.getElementById('confirm-settlement-btn').style.display = 'none'; document.getElementById('settlement-wait-msg').style.display = 'block'; };
...
dom.btns.payT.onclick = () => { let sels = document.querySelectorAll('.selected'); if(sels.length!==1)return alert("请选1张主牌进贡"); socket.emit('payTribute', myHand[parseInt(sels[0].dataset.index)]); dom.btns.payT.style.display='none'; };
dom.btns.retT.onclick = () => { let sels = document.querySelectorAll('.selected'); if(sels.length!==1)return alert("请选1张牌还供"); socket.emit('returnTribute', myHand[parseInt(sels[0].dataset.index)]); dom.btns.retT.style.display='none'; };
...
dom.btns.bury.onclick = () => {
    ...
    let buriedCards = ids.map(idx => myHand[idx]); ids.forEach(idx => myHand.splice(idx,1));
    socket.emit('buryCards', { buried: buriedCards, leftoverHand: myHand }); dom.btns.bury.style.display='none'; renderHand();
};
...
dom.btns.play.onclick = () => {
    ...
    ids.forEach(idx => myHand.splice(idx,1)); socket.emit('playCards', { played: cards, leftoverHand: myHand }); dom.btns.play.style.display='none'; renderHand();
};
...
socket.on('seatAssigned', d => { myIdx = d.seatIndex; myName = d.nickname; amIOwner = d.isOwner; amISpectator = false; dom.ident.innerText = `你是: ${myName}`; dom.ownerPan.style.display = amIOwner ? 'block' : 'none'; dom.playerPan.style.display = (!amIOwner) ? 'block' : 'none'; dom.specPan.style.display = 'none'; });
...
function updateAvatarUI() {
    for(let i=0; i<4; i++) {
        let diff = amISpectator ? i : (i - myIdx + 4) % 4; let pId = ['player-south','player-east','player-north','player-west'][diff];
        let pUI = document.getElementById(pId); if(!pUI) continue;
        ...
        if(i === currentTurnIdx && !sInfo?.isOffline && ['PLAYING', 'DRAWING', 'BURYING_TAKE', 'BURYING_ACTION'].includes(gState)) pUI.classList.add('active-turn'); else { pUI.classList.remove('active-turn'); pUI.querySelector('.timer-badge').innerText = '0'; }
    }
}
...
socket.on('roomStateSync', d => { 
    ...
    updateAvatarUI();
    if(amIOwner) {
        let seatedCount = 0, readyCount = 0; d.seats.forEach(s => { if(s){ seatedCount++; if(s.isReady || s.isOwner || s.isOffline) readyCount++; }});
...
socket.on('showSettlement', html => { dom.settlement.style.display = 'flex'; document.getElementById('settlement-details').innerHTML = html; document.getElementById('confirm-settlement-btn').style.display = 'inline-block'; document.getElementById('settlement-wait-msg').style.display = 'none'; });
socket.on('hideSettlement', () => { dom.settlement.style.display = 'none'; });

socket.on('gameStateSync', d => {
    gState=d.state; mainS=d.mainSuit; isFirstG=d.isFirstGame; onStagePlayers=d.onStage; tConfig=d.tribute;
    document.getElementById('current-game').innerText=d.match.currentGame; document.getElementById('team1-wins').innerText=d.match.team1Wins; document.getElementById('team2-wins').innerText=d.match.team2Wins;
    ...
    if (d.cardCounts) { for(let i=0; i<4; i++) { let diff = amISpectator ? i : (i - myIdx + 4) % 4; let pId = ['player-south','player-east','player-north','player-west'][diff]; if(pId !== 'player-south') { let pUI = document.getElementById(pId); if(pUI) pUI.querySelector('.card-count').innerText = d.cardCounts[i]; } } }
    renderHand(); updateAvatarUI();
});

socket.on('deckSync', d => { dom.deckArea.style.display = d.remain > 0 ? 'flex' : 'none'; dom.cardsRemain.innerText = d.remain; let shadowVal = Math.ceil(d.remain / 20); dom.pileL.style.boxShadow = `${shadowVal}px ${shadowVal}px 0 #95a5a6`; dom.pileR.style.boxShadow = `${shadowVal}px ${shadowVal}px 0 #95a5a6`; if (d.target) { dom.targetCardUI.style.display = 'flex'; let isRed = (d.target.suit==='♥'||d.target.suit==='♦'); dom.targetCardUI.innerHTML = `<span style="color:${isRed?'#d32f2f':'#333'}">${getVal(d.target)}<br>${d.target.suit}</span>`; } else dom.targetCardUI.style.display = 'none'; });
...
socket.on('showPub', c => { dom.pubArea.innerHTML = ''; c.forEach(card => { let div = document.createElement('div'); div.className = 'playing-card'; let isRed = (card.suit==='♥'||card.suit==='♦'||card.value==='大王'); div.innerHTML = `<div class="card-corner" style="color:${isRed?'#d32f2f':'#333'}"><span>${getVal(card)}</span><span>${card.suit==='Joker'?'王':card.suit}</span></div>`; dom.pubArea.appendChild(div); }); });
...
socket.on('playerPlayed', d => { if(trickClient.length===4) trickClient=[]; trickClient.push(d); let diff = amISpectator ? d.idx : (d.idx - myIdx + 4)%4; let slot = document.getElementById(['slot-south','slot-east','slot-north','slot-west'][diff]); slot.innerHTML = ''; d.cards.forEach(c => { let div = document.createElement('div'); div.className='playing-card'; let isRed = (c.suit==='♥'||c.suit==='♦'||c.value==='大王'); div.innerHTML = `<div class="card-corner" style="color:${isRed?'#d32f2f':'#333'}"><span>${getVal(c)}</span><span>${c.suit==='Joker'?'王':c.suit}</span></div>`; slot.appendChild(div); }); });
